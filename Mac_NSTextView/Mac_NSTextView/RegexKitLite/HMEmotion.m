...
{
    return [NSString stringWithFormat:@"%@ - %@ - %@ - %@", self.chs, self.gif, self.png, self.code];
}
...
- (BOOL)isEqual:(HMEmotion *)otherEmotion
{
    if (self.code) { // emoji表情
        ...
        return [self.png isEqualToString:otherEmotion.png] && [self.chs isEqualToString:otherEmotion.chs] && [self.cht isEqualToString:otherEmotion.cht]&&[self.gif isEqualToString:otherEmotion.gif];
    }
}
