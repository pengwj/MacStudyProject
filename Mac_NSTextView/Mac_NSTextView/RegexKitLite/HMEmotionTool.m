...
+ (NSArray *)defaultEmotions
{
    ...
    if (!_defaultEmotions) {
       // NSString *plist = [[NSBundle mainBundle] pathForResource:@"EmotionIcons/default/info.plist" ofType:nil];
        ...
        NSString *plist = [[NSBundle mainBundle] pathForResource:@"image.plist" ofType:nil];
       // NSString *plist = [NSString stringWithFormat:@"%@/%@",resourcePath,@"custom_Expression2.plist"];
        _defaultEmotions = [HMEmotion objectArrayWithFile:plist];
        //[_defaultEmotions makeObjectsPerformSelector:@selector(setDirectory:) withObject:@"EmotionIcons/default"];Contents/Resources
        [_defaultEmotions makeObjectsPerformSelector:@selector(setDirectory:) withObject:resourcePath];
    }
...
+ (NSArray *)lxhEmotions
{
    if (!_lxhEmotions) {
        ...
        [_lxhEmotions makeObjectsPerformSelector:@selector(setDirectory:) withObject:@"Contents/Resources"];
    }
    return _lxhEmotions;
}
